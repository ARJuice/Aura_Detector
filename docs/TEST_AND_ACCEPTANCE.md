...
| Overlay | Baseline built and installed; manual acceptance pending | Phone binds preview and analysis to one CameraX viewport, transmits the matching cropped/upright image, and uses the exact source-to-preview transform for the Canvas overlay. The rendered contour receives one 90-degree clockwise display calibration around the correctly aligned projected box. Verify it follows the person in portrait, then rotate to landscape and check again. |
