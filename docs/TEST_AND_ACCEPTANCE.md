...
| Android installation | Passed | Latest debug APK, including Stage 9 safeguards and the no-action-bar theme, installs on the authorized phone; the native `AUR/S` header is absent. |
| Latest-frame transport | Passed for private hotspot | Phone camera opens and reaches the PC over the Windows hotspot; after the Stage 9 install it reached `LINK: OK`, `FRAME: 44`, `65ms`, with one subject. Earlier it held `LINK: OK` through `FRAME: 130` after the CameraX guard. These are smoke observations, not p95 measurements. |
